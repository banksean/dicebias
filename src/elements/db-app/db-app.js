...
class AppElement extends LitElement {
    ...
    render(){
      ...
      for (let i = 0; i < this.sides; i++) {
          buttons.push(html`<mwc-button icon='add_circle' @click=${(e) => {this.numberClicked(i)}}>${i}</mwc-button>`);
      }
      buttons.push(html`<mwc-button icon='undo' @click=${this.undoClicked}>undo</mwc-button>`);
